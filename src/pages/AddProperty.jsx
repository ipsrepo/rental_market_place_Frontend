...
import {addProperty, updateProperty} from "../services/property.service.js";
    ...
    <div>
        ...
        <input
            className={`w-full px-3.5 py-2.5 border rounded-lg text-sm text-accent placeholder-gray-400
                ...
                ${error ? 'border-red bg-red/5' : 'border-border bg-white hover:border-gray-400'}`}
            {...props}
        />
...
const Toggle = ({label, description, checked, onChange}) => (
    <div className="flex items-center justify-between py-3 px-4 bg-bg rounded-lg border border-border cursor-pointer">
        <div>
            <p className="text-sm font-medium text-accent">{label}</p>
            {description && <p className="text-xs text-text mt-0.5">{description}</p>}
        </div>
        <button
            type="button"
...
const ImageUpload = ({images, onAdd, onRemove, maxImages = 10}) => {
    ...
    return (
        <div>
            <div className="grid grid-cols-3 gap-3">
                {images.map((img, i) => (
                    <div key={i}
                         className="relative aspect-video rounded-lg overflow-hidden border border-border bg-bg">
                        <img
                        ...
                        <button
                            type="button"
                            onClick={() => onRemove(i)}
                            className="absolute top-1 right-1 w-6 h-6 bg-red text-white rounded-full flex items-center justify-center text-xs font-bold hover:opacity-80 transition-opacity"
                        >
                            ×
                        </button>
                        {i === 0 && (
                ...
                {images.length < maxImages && (
                    <button
                        ...
                        className="aspect-video rounded-lg border-2 border-dashed border-border hover:border-primary hover:bg-bg transition-all flex flex-col items-center justify-center gap-1 text-text hover:text-primary"
                    >
            ...
            <p className="text-xs text-text opacity-60 mt-2">First image will be the primary image.
                Max {maxImages} photos.</p>
...
const PropertyFormPage = () => {
    ...
    const validate = () => {
        ...
        if (!form.price || Number(form.price) <= 0) e.price = 'Please enter a valid price';
        if (!form.propertytype) e.propertytype = 'Please select a property type';
        if (!form.availablefrom) e.availablefrom = 'Please select an available from date';
                ...
                <div className="max-w-3xl mx-auto px-4 py-4 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        <button
                            type="button"
                            onClick={() => nav(-1)}
                            className="w-8 h-8 rounded-lg border border-border flex items-center justify-center text-text hover:bg-bg transition-colors"
                        >
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <button
                            ...
                            disabled={saving}
                            className="bg-primary disabled:opacity-60 text-white text-sm font-medium px-5 py-2 rounded-lg transition-colors flex items-center gap-2"
                        >
                            {saving && (
            ...
            <form onSubmit={handleSubmit} className="max-w-3xl mx-auto px-4 py-8 space-y-6">
                ...
                <div className="bg-white rounded-2xl border border-border p-6 space-y-5">
                    ...
                    <div className="text-right -mt-3">
                        <span className="text-xs text-text opacity-60">{form.title.length}/150</span>
                    </div>
                    <div>
                        ...
                        <textarea rows={5} maxLength={2000}
                                ...
                                ${errors.details ? 'border-red bg-red/5' : 'border-border hover:border-gray-400'}`}
                        />
                        <div className="flex justify-between mt-1">
                            {errors.details ? <p className="text-xs text-red">{errors.details}</p> : <span/>}
                            <span className="text-xs text-text opacity-60">{form.details.length}/2000</span>
                        </div>
                    </div>
                    <Input label="Location" required placeholder="e.g. Rathmines, Dublin 6"
                ...
                <div className="bg-white rounded-2xl border border-border p-6 space-y-5">
                    ...
                    <div>
                        ...
                        {errors.propertytype && <p className="mt-1.5 text-xs text-red">{errors.propertytype}</p>}
                    </div>
                    <div>
                        <Label>Rental type</Label>
                        <div className="grid grid-cols-3 gap-3">
                            {RENTAL_TYPES.map((t) => (
                                <button key={t.value} type="button" onClick={() => set('rentaltype', t.value)}
                                        className={`py-3 px-4 rounded-xl border-2 text-sm font-medium transition-all cursor-pointer ${
                            ...
                            <div className="relative">
                                <input type="number" min="0" placeholder="0" value={form.price}
                                       ...
                                       className={`w-full pl-7 pr-3.5 py-2.5 border rounded-lg text-sm text-accent
                                        focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition-all
                                        ${errors.price ? 'border-red bg-red/5' : 'border-border hover:border-gray-400'}`}
                                />
                            </div>
                            {errors.price && <p className="mt-1 text-xs text-red">{errors.price}</p>}
                        </div>
                        <Select label="Price period" value={form.pricedisplay}
                                onChange={(e) => set('pricedisplay', e.target.value)}>
                            {PRICE_DISPLAY.map((p) => <option key={p.value} value={p.value}>{p.label}</option>)}
                        </Select>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-2xl border border-border p-6 space-y-5">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <Select label="Bedrooms" value={form.bedrooms}
                            ...
                            {BEDROOM_OPTIONS.map((n) => <option key={n}
                                                                value={n}>{n === 0 ? 'Studio (0)' : `${n} bedroom${n > 1 ? 's' : ''}`}</option>)}
                        </Select>
                        <Select label="Bathrooms" value={form.bathrooms}
                                onChange={(e) => set('bathrooms', Number(e.target.value))}>
                            {BATHROOM_OPTIONS.map((n) => <option key={n}
                                                                 value={n}>{n} bathroom{n > 1 ? 's' : ''}</option>)}
                        </Select>
                    </div>
                    <div className="space-y-3">
                        <Toggle label="Private bathroom" description="Tenant has exclusive use of a bathroom"
                                checked={form.isprivatebathroom} onChange={(v) => set('isprivatebathroom', v)}/>
                        <Toggle label="Shared bed" description="Bedroom is shared with another person"
                                checked={form.issharedbed} onChange={(v) => set('issharedbed', v)}/>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-2xl border border-border p-6 space-y-3">
                    ...
                    <Toggle label="Bills included" description="Electricity, gas, and/or internet included in rent"
                            checked={form.billsincluded} onChange={(v) => set('billsincluded', v)}/>
                    <Toggle label="Furnished" description="Property comes with furniture" checked={form.furnished}
                            onChange={(v) => set('furnished', v)}/>
                    <Toggle label="New listing" description="Mark this as a newly added property" checked={form.isnew}
                            onChange={(v) => set('isnew', v)}/>
                    <Toggle label="Available" description="Property is currently available to rent"
                ...
                <div className="bg-white rounded-2xl border border-border p-6 space-y-5">
                    ...
                    <Input label="Available from" required type="date" value={form.availablefrom}
                           onChange={(e) => set('availablefrom', e.target.value)} error={errors.availablefrom}/>
                    <Select label="BER Rating" value={form.berrating}
                            onChange={(e) => set('berrating', e.target.value)}>
                        <option value="">— Select BER rating —</option>
                        {BER_RATINGS.map((r) => <option key={r.value} value={r.value}>{r.label}</option>)}
                    </Select>
                ...
                <div className="flex gap-3 pb-8">
                    <button type="button" onClick={() => nav(-1)}
                            className="flex-1 py-3 border border-border text-text font-medium rounded-xl hover:bg-bg transition-colors">
                        Cancel
                    </button>
                    <button type="submit" disabled={saving}
                            className="flex-1 py-3 cursor-pointer bg-primary disabled:opacity-60 text-white font-medium rounded-xl transition-colors flex items-center justify-center gap-2">
                        {saving && (
                            <svg className="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24">
                                <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor"
                                        strokeWidth="4"/>
